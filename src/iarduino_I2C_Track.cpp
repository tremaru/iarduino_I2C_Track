...
bool	iarduino_I2C_Track::begin			(uint8_t type, uint8_t p1, uint8_t p2, uint8_t p3, uint8_t p4){						//	Параметры:				type - тип модуля (знак/светофор), p1 - группа знака или перекрёсток светофора, p2 - номер знака или секции светофора [,p3 - пункт знака] [, p4 - табличка знака].
...
bool	iarduino_I2C_Track::backlight		(uint8_t led_1, uint8_t led_2){														//	Параметры:				led_1 - 0=откл/1=вкл/2=мигать [, led_2 - 0=откл/1=вкл/2=мигать].
			...
			//	Определяем значения регистров управления цепями подсветки:														//
				data[0] = (led_1==2 ? mask_1:0) | (led_2==2 ? mask_2:0);														//	Определяем значение для регистра «BLINK_ON» устанавливающего   биты рпегистра «BLINK».
				data[1] = (led_1==2 ? 0:mask_1) | (led_2==2 ? 0:mask_2);														//	Определяем значение для регистра «BLINK_OFF» сбрасывающего     биты рпегистра «BLINK».
				data[2] = (led_1    ? mask_1:0) | (led_2    ? mask_2:0);														//	Определяем значение для регистра «DIGITAL_ON» устанавливающего биты рпегистра «DIGITAL».
				data[3] = (led_1    ? 0:mask_1) | (led_2    ? 0:mask_2);														//	Определяем значение для регистра «DIGITAL_OFF» сбрасывающего   биты рпегистра «DIGITAL».
