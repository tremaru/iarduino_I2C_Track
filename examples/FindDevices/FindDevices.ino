    ...
    j=Wire.endTransmission();                                                 //   Выполняем инициированную ранее передачу данных.
      if(j==0){                                                               //   Если передача выполнена успешно (0-передача успешна / 1 - переполнен буфер для передачи / 2 - получен NACK при передаче адреса / 3 - получен NACK при передаче данных / 4 - другая ошибка), то ...
        ...
        if(((k[2]>>1)==i) && (k[3]==0xC3 || k[3]==0x3C)){                     //   Если значение k[2] (без последнего бита) совпадает с адресом устройства i, а значение k[3] совпадает с идентификатором 0xC3 (модуль «Metro»), или 0x3С (модуль «Flash»), то ...
          ...
          if(k[0]==0x17){Serial.print(F("ИК-приёмопередатчик"       )); }else //   Если в 0 элементе массива k хранится значение 0x17, значит это ИК-приёмопередатчик.
          if(k[0]==0x18){Serial.print(F("ИК-модуль трассы"          )); }else //   Если в 0 элементе массива k хранится значение 0x18, значит это ИК-модуль трассы.
          if(k[0]==0x19){Serial.print(F("TDS/EC-метр"               )); }else //   Если в 0 элементе массива k хранится значение 0x19, значит это TDS/EC-метр.
          if(k[0]==0x1A){Serial.print(F("pH-метр"                   )); }else //   Если в 0 элементе массива k хранится значение 0x1A, значит это pH-метр.
