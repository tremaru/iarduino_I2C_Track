...
void setup(){                                                       //
     ...
     tlight.begin(&sWire, MODUL_TLIGHT, TRACK_LRF, SECTION_LR);     //   Инициируем работу с модулем. Указываем ссылку на объект для работы с шиной I2C, модуль является светофором, дорога прямо налево и направо, с двумя секциями поворотов.
//   tlight.begin(&sWire, MODUL_TLIGHT, TRACK_FLR, SECTION_RL);     //   Инициируем работу с модулем. Указываем ссылку на объект для работы с шиной I2C, модуль является светофором, дорога прямо налево и направо, с двумя секциями поворотов.
//   tlight.begin(&sWire, MODUL_TLIGHT, TRACK_FRL, SECTION_LR);     //   Инициируем работу с модулем. Указываем ссылку на объект для работы с шиной I2C, модуль является светофором, дорога прямо налево и направо, с двумя секциями поворотов.
